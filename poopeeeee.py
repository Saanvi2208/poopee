...
st.set\_page\_config(page\_title="CleanTrust", page\_icon="◉", layout="wide", initial\_sidebar\_state="expanded")

load\_dotenv("/app/frontend/.env")
API\_URL = os.environ["REACT\_APP\_BACKEND\_URL"] + "/api"
...
def inject\_css():
    st.markdown("""
    \<style>
    @import url('[https://fonts.googleapis.com/css2?family=DM+Sans\:wght@400;500;600;700&family=Space+Grotesk\:wght@500;600;700&display=swap'](https://fonts.googleapis.com/css2?family=DM+Sans\:wght@400;500;600;700\&family=Space+Grotesk\:wght@500;600;700\&display=swap%27));
    ...
    .pill.warn { background:#fff2d8; color:#9b6514; }.pill.bad { background:#ffe4e3;color:#b23b36; }.pill.gray{background:#e8edf1;color:#687684;}
    ...
    .map-title { padding:18px; font-weight:700; color:#24565a; }.marker { position\:absolute; background:#fff; width:38px;height:38px;border-radius:50% 50% 50% 0; transform\:rotate(-45deg); box-shadow:0 4px 12px #13504b40; display\:flex;align-items\:center;justify-content\:center; }.marker b{transform\:rotate(45deg);font-size:14px}.m1{left:18%;top:42%}.m2{left:45%;top:25%}.m3{left:69%;top:47%}.m4{left:38%;top:69%}.m5{left:78%;top:22%}.m6{left:59%;top:76%}
    ...
    def auth\_screen():
        st.markdown('\<div class="brand">\<span class="brand-mark">◉\</span>\<b style="font-size:22px">CleanTrust\</b>\<div style="color:#9db9b6;font-size:12px;margin-top:7px">The civic sanitation network\</div>\</div>', unsafe\_allow\_html=True)
        \_, col, \_ = st.columns([1, 1.2, 1])
        with col:
            st.markdown('\<div class="eyebrow">WELCOME TO CLEANTRUST\</div>\<h1 style="font-size:42px">Trust, before you go.\</h1>\<p style="color:#65758b">Sign in to find cleaner public spaces, report issues, or connect with local sanitation work.\</p>', unsafe\_allow\_html=True)
            email = st.text\_input("Email", value="demo\@cleantrust.city", key="login-email")
            password = st.text\_input("Password", value="demo123", type="password", key="login-password")
            role = st.selectbox("I am joining as", ["Citizen", "Sanitation worker", "Facility manager"], key="login-role")
            if st.button("Enter CleanTrust", type="primary", use\_container\_width=True, key="login-submit"):
                api\_role = {"Citizen": "citizen", "Sanitation worker": "worker", "Facility manager": "admin"}[role]
                try:
                    response = [requests.post](http://requests.post/)(f"{API\_URL}/auth/register", json={"email": email, "password": password, "role": api\_role}, timeout=8)
                    if response.status\_code == 409:
                        response = [requests.post](http://requests.post/)(f"{API\_URL}/auth/login", json={"email": email, "password": password, "role": api\_role}, timeout=8)
                        response.raise\_for\_status()
                        st.session\_state.user = {"email": email, "role": role, "token": response.json().get("token")}
                        [st.session\_state.page](http://st.session_state.page/) = "Home"
                        st.rerun()
                        ...
                        def score\_class(value):
                            return "" if value >= 80 else "warn" if value >= 60 else "bad"


                            def toilet\_card(t, compact=False):
                                status = "OPEN" if t["open"] else "CLOSED"
                                status\_class = "" if t["open"] else "gray"
                                st.markdown(f'''\<div class="toilet">\<div style="display\:flex;justify-content\:space-between;gap:10px">\<div>\<div class="eyebrow">PUBLIC TOILET · {t["area"]}\</div>\<h3 style="margin:5px 0">{t["name"]}\</h3>\<span class="pill {status\_class}">{status}\</span>\<span class="pill {score\_class(t["trust"])}">{t["cleanliness"]}\</span>\<span class="pill">{t["water"]}\</span>\</div>\<div style="text-align\:right">\<div class="score">{t["trust"]}\<small style="font:500 12px DM Sans;color:#65758b"> / 100\</small>\</div>\<small style="color:#65758b">TRUST INDEX\</small>\</div>\</div>\<div style="border-top:1px solid #edf2f1;margin:14px 0 12px">\</div>\<div style="display\:flex;justify-content\:space-between;color:#65758b;font-size:13px">\<span>✓ Verified {t["cleaned"]}\</span>\<span>◌ {t["distance"]}\</span>\<span>{" Accessible" if t["accessible"] else "— Not accessible"}\</span>\</div>\</div>''', unsafe\_allow\_html=True)


                                def home():
                                    st.markdown('\<div class="hero">\<div class="eyebrow">GOOD MORNING, MAYA · LIVE CITY VIEW\</div>\<h1>Know if you can trust it.\</h1>\<p>Public sanitation, made visible. Every score is built from condition, freshness, reports, and verified cleaning.\</p>\</div>', unsafe\_allow\_html=True)
                                    st.markdown('\<div class="section">Your city at a glance\</div>', unsafe\_allow\_html=True)
                                    cols = st.columns(4)
                                    ...
                                    for c, (v, label) in zip(cols, stats):
                                        with c: st.markdown(f'\<div class="stat">\<div class="eyebrow">CLEANTRUST LIVE\</div>\<div class="value">{v}\</div>\<div style="color:#65758b;font-size:13px">{label}\</div>\</div>', unsafe\_allow\_html=True)
                                        st.markdown('\<div class="section">Nearest facilities \<span style="font:500 13px DM Sans;color:#65758b">· Demo location: Fort District\</span>\</div>', unsafe\_allow\_html=True)
                                        left, right = st.columns([1.15, 1])
                                        with left:
                                            st.markdown('\<div class="map">\<div class="map-title">FORT DISTRICT · 8 facilities nearby\</div>\<div class="marker m1">\<b style="color:#10a878">91\</b>\</div>\<div class="marker m2">\<b style="color:#10a878">84\</b>\</div>\<div class="marker m3">\<b style="color:#e99b38">73\</b>\</div>\<div class="marker m4">\<b style="color:#e44d45">52\</b>\</div>\<div class="marker m5">\<b style="color:#10a878">88\</b>\</div>\<div class="marker m6">\<b style="color:#e99b38">79\</b>\</div>\</div>', unsafe\_allow\_html=True)
                                            st.caption("◉ Map markers show Trust Index · Location is simulated for this demo")
                                            if st.button("Find nearest toilet", type="primary", use\_container\_width=True, key="find-nearest"):
                                                st.session\_state.selected = TOILETS[0]["name"]
                                                st.toast("Central Market is 450 m away · Trust 91")
                                                with right:
                                                    for t in TOILETS[:2]:
                                                        toilet\_card(t)
                                                        st.markdown('\<div class="section">What needs attention\</div>', unsafe\_allow\_html=True)
                                                        st.markdown('\<div class="notice">\<b>2 reports need a response\</b>\<br>Harbor View has 8 recent reports, including no water. A maintenance shift is available nearby.\</div>', unsafe\_allow\_html=True)


                                                        def toilets\_page():
                                                            st.markdown('\<div class="eyebrow">DISCOVER · VERIFY · GO\</div>\<h1>Nearby toilets\</h1>\<p style="color:#65758b">Compare the signals that make a public facility trustworthy.\</p>', unsafe\_allow\_html=True)
                                                            search = st.text\_input("Search by place or area", placeholder="Try ‘market’ or ‘station’", key="toilet-search")
                                                            visible = [t for t in TOILETS if search.lower() in (t["name"] + t["area"]).lower()]
                                                            for t in visible:
                                                                toilet\_card(t)
                                                                a, b, c = st.columns([1, 1, 2])
                                                                with a:
                                                                    if st.button("View details", key=f"view-{t['name']}"): st.session\_state.detail = t["name"]
                                                                    with b:
                                                                        if st.button("Navigate", key=f"nav-{t['name']}"): st.toast(f"Demo route to {t['name']} opened")
                                                                        with c:
                                                                            if st.button("Report issue", key=f"report-{t['name']}"): st.session\_state.report\_toilet = t["name"]; [st.session\_state.page](http://st.session_state.page/) = "Report issue"; st.rerun()
                                                                            if st.session\_state.get("detail"):
                                                                                t = next(x for x in TOILETS if x["name"] == st.session\_state.detail)
                                                                                st.markdown(f'\<div class="section">{t["name"]} · transparent score\</div>\<div class="stat">\<b>Condition {t["condition"]}/100\</b> &nbsp; \<b>Freshness {t["freshness"]}/100\</b> &nbsp; \<b>Trust {t["trust"]}/100\</b>\<br>\<small style="color:#65758b">Trust = cleanliness 25% · water 20% · availability 15% · maintenance 15% · safety 10% · accessibility 5% · information freshness 10%\</small>\</div>', unsafe\_allow\_html=True)


                                                                                def report\_page():
                                                                                    st.markdown('\<div class="eyebrow">CITIZEN SIGNAL\</div>\<h1>Report an issue\</h1>\<p style="color:#65758b">Your report helps a worker respond before the next person arrives.\</p>', unsafe\_allow\_html=True)
                                                                                    names = [t["name"] for t in TOILETS]
                                                                                    toilet = st.selectbox("Toilet", names, index=names.index(st.session\_state.get("report\_toilet", names[0])), key="report-toilet")
                                                                                    issue = st.selectbox("Issue type", ["Dirty toilet", "No water", "Broken toilet", "Overflowing waste", "Bad smell", "No lighting", "Accessibility problem", "Toilet closed", "Other"], key="report-issue")
                                                                                    image = st.file\_uploader("Upload photo (optional)", type=["png", "jpg", "jpeg"], key="report-image")
                                                                                    description = st.text\_area("What did you notice?", placeholder="Add useful details for the response team", key="report-description")
                                                                                    if image:
                                                                                        [st.info](http://st.info/)("AI-assisted analysis · Possible cleanliness issue detected. Human verification recommended.")
                                                                                        if st.button("Submit report", type="primary", key="submit-report"):
                                                                                            st.session\_state.report\_submitted = True
                                                                                            st.session\_state.report\_count = st.session\_state.get("report\_count", 0) + 1
                                                                                            st.success(f"Report received for {toilet}. A response team has been notified.")


                                                                                            def jobs\_page():
                                                                                                st.markdown('\<div class="eyebrow">LOCAL OPPORTUNITIES\</div>\<h1>Find sanitation work\</h1>\<p style="color:#65758b">Fair, nearby work that keeps public spaces running.\</p>', unsafe\_allow\_html=True)
                                                                                                st.markdown('\<div class="notice">\<b>Worker profile strength: 86%\</b> · Add availability to get matched with more shifts\</div>', unsafe\_allow\_html=True)
                                                                                                st.markdown('\<div class="section">Recommended near you\</div>', unsafe\_allow\_html=True)
                                                                                                for job in JOBS:
                                                                                                    st.markdown(f'\<div class="toilet">\<div style="display\:flex;justify-content\:space-between">\<div>\<span class="pill">{job["tag"]}\</span>\<h3>{job["title"]}\</h3>\<p style="color:#65758b;margin:0">{job["place"]} · {job["time"]}\</p>\</div>\<div style="text-align\:right">\<div class="score" style="font-size:27px;color:#122033">{job["pay"]}\</div>\<small>{job["distance"]} away\</small>\</div>\</div>\</div>', unsafe\_allow\_html=True)
                                                                                                    if st.button("Apply for this job", key=f"apply-{job['title']}"):
                                                                                                        ...
                                                                                                        def admin\_page():
                                                                                                            st.markdown('\<div class="eyebrow">FACILITY MANAGEMENT\</div>\<h1>Sanitation command center\</h1>\<p style="color:#65758b">A live operating view for a cleaner Fort District.\</p>', unsafe\_allow\_html=True)
                                                                                                            cols = st.columns(4)
                                                                                                            for c, (v, label, tone) in zip(cols, [("148", "monitored toilets", ""), ("19", "critical", "bad"), ("24", "active jobs", "warn"), ("96%", "completion rate", "")]):
                                                                                                                with c: st.markdown(f'\<div class="stat">\<div class="score {tone}" style="font-size:30px">{v}\</div>\<div style="color:#65758b;font-size:13px">{label}\</div>\</div>', unsafe\_allow\_html=True)
                                                                                                                st.markdown('\<div class="section">Today’s response queue\</div>', unsafe\_allow\_html=True)
                                                                                                                st.dataframe({"Priority": ["Critical", "High", "Medium", "High"], "Facility": ["Harbor View", "West Bazaar", "Civic Square", "Central Market"], "Signal": ["No water · 8 reports", "Closed · 11 reports", "Low pressure", "Cleanliness report"], "Status": ["Job available", "Assigned", "Monitoring", "New"]}, use\_container\_width=True, hide\_index=True)
                                                                                                                st.markdown('\<div class="section">Post a sanitation job\</div>', unsafe\_allow\_html=True)
                                                                                                                a, b = st.columns(2)
                                                                                                                with a: st.selectbox("Facility", [t["name"] for t in TOILETS], key="admin-facility"); st.selectbox("Job type", ["Cleaning", "Maintenance", "Water inspection"], key="admin-type")
                                                                                                                with b: st.number\_input("Payment (₹)", min\_value=200, value=600, step=50, key="admin-payment"); st.text\_input("Start time", value="7:00 PM", key="admin-time")
                                                                                                                if st.button("Post job", type="primary", key="post-job"):
                                                                                                                    ...
                                                                                                                    def main():
                                                                                                                        inject\_css()
                                                                                                                        if "user" not in st.session\_state:
                                                                                                                            auth\_screen(); return
                                                                                                                            with st.sidebar:
                                                                                                                                st.markdown('\<div class="brand">\<span class="brand-mark">◉\</span>\<b style="font-size:20px">CleanTrust\</b>\<div style="color:#9db9b6;font-size:12px;margin-top:7px">Fort District · Live demo\</div>\</div>', unsafe\_allow\_html=True)
                                                                                                                                role = st.session\_state.user["role"]
                                                                                                                                st.caption(f"SIGNED IN AS {role.upper()}")
                                                                                                                                options = ["Home", "Nearby toilets", "Report issue", "Find work", "Notifications"] if role != "Facility manager" else ["Home", "Command center", "Nearby toilets", "Notifications"]
                                                                                                                                ...
                                                                                                                                if st.button("Sign out", key="sign-out"):
                                                                                                                                    st.session\_state.clear(); st.rerun()
                                                                                                                                    if page == "Home": home()
                                                                                                                                elif page == "Nearby toilets": toilets\_page()
                                                                                                                            elif page == "Report issue": report\_page()
                                                                                                                        elif page == "Find work": jobs\_page()
                                                                                                                    elif page == "Command center": admin\_page()
...
def main():
    ...
    # Safe state synchronization for cross-page navigation targets
